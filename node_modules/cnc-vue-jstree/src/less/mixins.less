.tree-theme (@base-height, @image, @image-height) {
	...
	.tree-icon-folder { background-position:-(@image-height * 8 + @correction) -@correction; }
	.tree-icon-file { background-position:-(@image-height * 3 + @correction) -(@image-height * 2 + @correction); }
	.tree-icon-formselect { background-position:-(@image-height * 4 + @correction) -(@image-height * 2 + @correction); }
	.tree-icon-forminput { background-position:-(@image-height * 5 + @correction) -(@image-height * 2 + @correction); }
	.tree-icon-formpassword { background-position:-(@image-height * 6 + @correction) -(@image-height * 2 + @correction); }
	.tree-icon-formdate { background-position:-(@image-height * 7 + @correction) -(@image-height * 2 + @correction); }
	.tree-icon-formtabs { background-position:-(@image-height * 8 + @correction) -(@image-height * 2 + @correction); }
	// background-position: -100px -69px;
	.tree-icon-prep { width: 24px; height: 24px; line-height: 24px; display: inline-block; margin: 0;
    padding: 0;
    vertical-align: top;
    text-align: center;}
	...
	.treeitem-actions button:active {
	outline: none;
	border: none;
	}
	.tree-hovered .treeitem-actions {
		visibility: visible;
	}
}
